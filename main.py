...
async def upload_photo_to_drive(photo_bytes: BytesIO, user_fio: str) -> Optional[str]:
    try:
        ...
        media = MediaIoBaseUpload(photo_bytes, mimetype='image/jpeg', resumable=True)
...
async def start_report(message: Message, state: FSMContext):
    ...
    builder.adjust(2)
    await message.answer("<b>Шаг 1/4.</b> Выберите выполненный процесс:", reply_markup=builder.as_markup())
...
async def process_chosen(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(
        f"<b>Шаг 2/4.</b> Теперь детально опишите проделанную работу.\n\n{example_text}"
    )
    await callback.answer()

@router.message(ReportState.enter_work_description)
async def work_description_entered(message: Message, state: FSMContext):
    ...
    await message.answer("<b>Шаг 3/4.</b> Если в процессе работы возникли проблемы (не было заготовок, отключили свет, неверный чертеж), опишите их. Если проблем не было, нажмите 'Пропустить'.", reply_markup=skip_keyboard())

@router.message(ReportState.enter_problem_description)
async def problem_description_entered(message: Message, state: FSMContext):
    ...
    await message.answer("<b>Шаг 4/4.</b> Прикрепите одну фотографию выполненной работы. Если фото не требуется, нажмите 'Пропустить'.", reply_markup=skip_keyboard())
...
async def photo_attached(message: Message, state: FSMContext):
    await message.answer("Сохраняю ваш отчет, пожалуйста, подождите...", reply_markup=ReplyKeyboardRemove())
