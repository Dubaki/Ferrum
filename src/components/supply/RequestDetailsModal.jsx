...
export default function RequestDetailsModal({ request, userRole, supplyActions, onClose }) {
  ...
  const getMainActionLabel = () => {
    if (canSubmitForApproval || canApproveTechnologist || canApproveShopManager || canApproveDirector) return 'Согласовать';
    if (canMarkPaid) return 'Оплачено';
    if (canSetDelivery) return 'Срок доставки';
        ...
        <div className="p-3 space-y-2 overflow-y-auto flex-1">
          ...
          {canAttachInvoice && (
            <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-3">
              ...
              {uploadError && (
                <div className="text-red-600 text-xs mt-2">{uploadError}</div>
              )}
            </div>
          )}
          ...
          <div className="grid grid-cols-2 gap-2 text-xs">
            ...
            {request.deliveryDate && (
              <div className="bg-cyan-50 p-2 rounded-lg">
                <div className="text-cyan-500 mb-0.5">Срок доставки</div>
                <div className="font-medium text-cyan-700">{formatDate(request.deliveryDate)}</div>
              </div>
            )}
            {request.deliveredAt && (
              <div className="bg-green-50 p-2 rounded-lg">
                <div className="text-green-500 mb-0.5">Доставлено</div>
                <div className="font-medium text-green-700">{formatDate(request.deliveredAt)}</div>
              </div>
            )}
            <div className="bg-slate-50 p-2 rounded-lg">
              <div className="text-slate-400 mb-0.5">Создано</div>
              <div className="font-medium text-slate-700">{formatDateTime(request.createdAt)}</div>
            </div>
          </div>

          {/* Согласования - компактно */}
            ...
            <div className="flex flex-wrap gap-1.5">
              {request.approvals?.technologist && (
                <span className="inline-flex items-center gap-1 px-2 py-1 bg-emerald-100 text-emerald-700 rounded text-xs font-medium">
                  <Check size={10} /> Технолог
                </span>
              )}
              {request.approvals?.shopManager && (
                <span className="inline-flex items-center gap-1 px-2 py-1 bg-emerald-100 text-emerald-700 rounded text-xs font-medium">
                  <Check size={10} /> Нач. цеха
                </span>
              )}
              {request.approvals?.director && (
                <span className="inline-flex items-center gap-1 px-2 py-1 bg-emerald-100 text-emerald-700 rounded text-xs font-medium">
                  <Check size={10} /> Директор
                </span>
              )}
              {request.approvals?.accountant && (
                <span className="inline-flex items-center gap-1 px-2 py-1 bg-emerald-100 text-emerald-700 rounded text-xs font-medium">
                  <Check size={10} /> Бухгалтер
                </span>
              )}
            </div>
          )}

          {/* История - сворачиваемый */}
          {request.statusHistory && request.statusHistory.length > 0 && (
            <div className="border border-slate-200 rounded-lg overflow-hidden">
              ...
              >
                <span className="text-sm font-medium text-slate-700 flex items-center gap-2">
                  <History size={14} />
                  История ({request.statusHistory.length})
                </span>
